...
class RegionInspectorPanel:
    ...
    def render(self, region_id: Optional[int], state: GameState, on_focus_request: Callable[[int, float, float], None]):
        ...
        if imgui.collapsing_header("Region Finder", True):
            ...
            # Refresh Cache if needed (or if list is empty but data might be ready)
            ...
            if imgui.begin_list_box("##region_list", (-1, 150)):
                ...
                for item in self._cached_list:
                    ...
                    # Display Format: "RegionName (OwnerTag)"
        ...
        try:
            ...
            if row_df.is_empty():
                imgui.text_colored((1, 0, 0, 1), "Region Data Not Found")
                imgui.end()
                return
            ...
            # Focus Button (Duplicate access for convenience)
            imgui.same_line()
    ...
    def _update_filter_cache(self, state: GameState, filter_text: str):
        ...
        try:
            ...
            # Filter columns: ID, Name, Owner, X, Y
            cols = ["id", "name", "owner", "center_x", "center_y"]

            if not txt:
                # Default: Show first 50 sorted by name
                ...
                ).select(cols).head(50) # Limit to 50 for performance
